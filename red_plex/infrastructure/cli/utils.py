        ...
        try:
            ...
            if not plex_collection:
                logger.warning('Plex collection with rating key "%s" not found. '
                               ...
                               'To fix this, you can:\n'
                               '  1. Re-run "convert" command for collage '
                               'ID %s to recreate the collection\n'
            ...
            group_ids = local_database.get_group_ids_by_rating_keys(current_rating_keys,
                                                                    collage.site.upper())

            if not group_ids:
                logger.warning('No group ID mappings found for collection '
                               '"%s" rating keys on site %s. '
            ...
            if not missing_group_ids:
                logger.info('Collage "%s" is already up to date (no new items to add)',
                            collage.name)
                success_count += 1
                continue
            ...
            if add_result and add_result.get('status') == 'success':
                ...
                logger.info('Successfully synced collage "%s": %d added, '
